...
-(void)createTabBarController{
    ...
    [self createViewControllersFor:tbc];
    self.window.rootViewController = tbc;
}
-(void)createViewControllersFor:(UITabBarController *)tbc{
    
    NSArray *ary = @[@"HeaderTwing",@"Performance"];
    NSArray *title = @[@"头条",@"段子"];
    tbc.tabBar.backgroundColor = [UIColor grayColor];
    for (NSUInteger i=0; i<ary.count; i++) {
        Class cls =NSClassFromString(ary[i]);
        UIViewController *vc=[[cls alloc] init];
        UITabBarItem *item  = [[UITabBarItem alloc] init];
        vc.tabBarItem = item;
        UINavigationController *nc =[[UINavigationController alloc] initWithRootViewController:vc];
        nc.tabBarItem.title = title[i];
